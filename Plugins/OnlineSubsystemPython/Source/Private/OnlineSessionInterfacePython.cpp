...
bool FOnlineSessionPython::CreateSession(int32 HostingPlayerNum, FName SessionName, const FOnlineSessionSettings& NewSessionSettings)
{
	...
	if (Session == NULL)
	{
		...
		else
		{
			...
			Request->SetVerb("GET");
			FString ServerName, MapName, GameMode, Version;
			...
			Session->SessionSettings.Get("VERSION", Version);
			...
			int port = SessionInfo->HostAddr->GetPort();

			UWorld* World = GetWorldForOnline(PythonSubsystem->GetInstanceName());
			if (port == 0) {
				#if WITH_ENGINE
					if (GEngine)
					{
			...
			Request->SetURL(
				FString::Printf(TEXT("http://%s/register_server?name=%s&port=%d&maxplayers=%d&pwprotected=%s&gamemode=%s&map=%s&version=%s&dedicated=%s"), 
				*config->ServerAddress, 
			    *FGenericPlatformHttp::UrlEncode(ServerName), 
				port,
				Session->NumOpenPublicConnections, 
				*FGenericPlatformHttp::UrlEncode(bPasswordProtected ? TEXT("true") : TEXT("false")), 
				*FGenericPlatformHttp::UrlEncode(GameMode),
				*FGenericPlatformHttp::UrlEncode(MapName),
				*FGenericPlatformHttp::UrlEncode(Version),
				*FGenericPlatformHttp::UrlEncode((World->GetNetMode() == ENetMode::NM_DedicatedServer) ? TEXT("true") : TEXT("false"))
...
bool FOnlineSessionPython::UpdateSession(FName SessionName, FOnlineSessionSettings& UpdatedSessionSettings, bool bShouldRefreshOnlineData)
{
	...
	if (Session)
	{
		...
		*/
		...
		uint32 PlayerCount = World->GetGameInstance()->GetNumLocalPlayers(); 
		UOnlineSubsystemPythonConfig* config = GetMutableDefault<UOnlineSubsystemPythonConfig>();
		auto addr = FString::Printf(TEXT("http://%s/update_server?name=%s&port=%d&maxplayers=%d&pwprotected=%s&gamemode=%s&map=%s&playercount=%d"),
			*config->ServerAddress,
			*FGenericPlatformHttp::UrlEncode(ServerName),
			World->URL.Port,
			...
			*FGenericPlatformHttp::UrlEncode(bPasswordProtected ? TEXT("true") : TEXT("false")),
			*FGenericPlatformHttp::UrlEncode(GameMode),
			*FGenericPlatformHttp::UrlEncode(MapName), PlayerCount);
		//UE_LOG_ONLINE_SESSION(Error, TEXT(*addr));
...
void FOnlineSessionPython::Heartbeat_ResponseReceived(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bWasSuccessful) {
	if (!Response.IsValid())
	{ 
	...
	if (FJsonSerializer::Deserialize(Reader, JsonObject))
	{
		...
		if (!bError)
		{
			...
			UE_LOG_ONLINE_SESSION(Warning, TEXT("Heartbeat OK!"));
		}
		else
		{
			TriggerOnUpdateSessionCompleteDelegates(CreateSessionName, false);
			FString ErrorMessage = JsonObject->GetStringField("message");
			UE_LOG_ONLINE_SESSION(Warning, TEXT("Error on heart beat! %s"), *ErrorMessage);

			// Seems like the host has restarted
			// Re-register  
			FNamedOnlineSession* Session = GetNamedSession(CreateSessionName);
			FHttpModule* Http = &FHttpModule::Get();
			auto Reregister_Request = Http->CreateRequest();
			Reregister_Request->SetHeader(TEXT("User-Agent"), TEXT("X-UnrealEngine-Agent"));
			Reregister_Request->SetHeader(TEXT("Content-Type"), TEXT("application/json"));
			//Request->SetHeader(TEXT("Authorization"), "Basic " + APIKey);
			Reregister_Request->SetVerb("GET");
			FString ServerName, MapName, GameMode, Version;
			Session->SessionSettings.Get("SERVERNAME", ServerName);
			Session->SessionSettings.Get("MAPNAME", MapName);
			Session->SessionSettings.Get("GAMEMODE", GameMode);
			...
			Reregister_Request->OnProcessRequestComplete().BindRaw(this, &FOnlineSessionPython::CreateSession_ResponseReceived);
			Reregister_Request->ProcessRequest();
		}
	}
}
	...
	if (FJsonSerializer::Deserialize(Reader, JsonObject))
	{
		...
		if (!bError)
		{
			...
			for (int32 i = 0; i != JsonServerList.Num(); i++)
			{
				...
				SearchResult.Session.SessionSettings.Set("IP", *server->GetStringField("ip"));
				SearchResult.Session.SessionSettings.Set(SETTING_MAPNAME, *server->GetStringField("map"));
				SearchResult.Session.SessionSettings.Set(SETTING_GAMEMODE, *server->GetStringField("gamemode"));
				SearchResult.Session.SessionSettings.Set("VERSION", *server->GetStringField("version"));
				...
				SearchResult.Session.SessionSettings.Set("MAXPLAYERS", server->GetIntegerField("maxplayers"));
				SearchResult.Session.SessionSettings.Set("DEDICATED", server->GetBoolField("dedicated"));
...
bool FOnlineSessionPython::JoinSession(int32 PlayerNum, FName SessionName, const FOnlineSessionSearchResult& DesiredSession)
{
	...
	if (Session == NULL)
	{
		...
		Session->SessionSettings.Get("ip",ipPunch);
		if (Return != ONLINE_IO_PENDING)
		{
	...
	bool dedicated;
	Session->SessionSettings.Get("dedicated", dedicated);
...
void FOnlineSessionPython::StartHeartbeat(float DeltaBetweenHeartbeats)
{
	...
	if (GEngine)
	{
		...
		World->GetTimerManager().SetTimer(PerformHeartbeat_Handle, FTimerDelegate::CreateRaw(this, &FOnlineSessionPython::PerformHeartbeat), 10.0, true, 1.0f);
	}
#endif
}

void FOnlineSessionPython::StartStunHeartbeat()
{
#if WITH_ENGINE
	if (GEngine)
	{
		...
		if (World->GetNetMode() != ENetMode::NM_DedicatedServer) {
			World->GetTimerManager().SetTimer(PerformStunHeartbeat_Handle, FTimerDelegate::CreateRaw(this, &FOnlineSessionPython::PerfromStunHeartbeat), 1, true, 1.0f);
		}
	}
#endif
}

void FOnlineSessionPython::StartPunchRequest()
{
#if WITH_ENGINE
	if (GEngine)
	{
		...
		World->GetTimerManager().SetTimer(PerformStunRequest_Handle, FTimerDelegate::CreateRaw(this, &FOnlineSessionPython::RequestPunchThrough), 1, true, 1.0f);
	}
#endif
}
...
void FOnlineSessionPython::PerformHeartbeat()
{
	...
	if (Session != nullptr) {
		...
		uint32 PlayerCount = World->GetGameState()->PlayerArray.Num();

		Request->SetURL(FString::Printf(TEXT("http://%s/perform_heartbeat?port=%d&playercount=%d"), *config->ServerAddress, port, PlayerCount));

		Request->OnProcessRequestComplete().BindRaw(this, &FOnlineSessionPython::Heartbeat_ResponseReceived);
		Request->ProcessRequest();
	}
	else {
		const FOnlineSubsystemPython& Subsystem = *PythonSubsystem;
	...
	//UpdateSession(CreateSessionName, Session->SessionSettings);
}

void FOnlineSessionPython::PerfromStunHeartbeat()
{
	UWorld* World = GetWorldForOnline(PythonSubsystem->GetInstanceName());
	...
	for (const FWorldContext& PieContext : GEngine->GetWorldContexts()) {
		for (auto driver : PieContext.ActiveNetDrivers) {
			UNetDriver * netDriver = driver.NetDriver;
		 }
	}
	...
	if (World) {
		if (!stunsock) {
			...
			if (netDriver) { 
				...
				stunsock = netDriver->GetSocketSubsystem()->CreateUniqueSocket(NAME_DGram, TEXT("Unreal"), NAME_None);

				auto localStunAddr = SocketSubSystem->CreateInternetAddr();
				localStunAddr->SetPort(12001);

				int32 BoundPort = SocketSubSystem->BindNextPort(stunsock.Get(), *localStunAddr, 1 + 1, 1);

				STUNReceiver = new FUdpSocketReceiver(stunsock.Get(), FTimespan(0, 0, 1), TEXT("LISTEN STUN SOCK"));

				FOnSocketDataReceived& delegate = STUNReceiver->OnDataReceived();

				static TSharedRef<FOnlineSessionPython> delegatePtr = TSharedRef<FOnlineSessionPython>(this);
				delegate.BindSP(delegatePtr, &FOnlineSessionPython::RecvStunMsg); 
		...
		if (stunsock) {
			...
			if (netDriver) {
				ISocketSubsystem* SocketSubSystem = netDriver->GetSocketSubsystem();
				auto STUNserverAddr = SocketSubSystem->CreateInternetAddr();
				...
				FString ipString = "89.46.81.246";
				bool isValid;

				STUNserverAddr->SetIp(*ipString, isValid);
				STUNserverAddr->SetPort(12000);

				if (isValid) {
					const char* msg =  "pulse";
					FSocket* temp = stunsock.Get();
					int32 bytesSent = 0;
					temp->SendTo((uint8*)msg, 6, bytesSent, STUNserverAddr.Get());
				}
			}
		}
	}
...
void FOnlineSessionPython::RecvStunMsg(const FArrayReaderPtr& ArrayReaderPtr, const FIPv4Endpoint& EndPt)
{
	//GEngine->AddOnScreenDebugMessage(-4, 1, FColor::Red, ipString);
	...
	if (World) {
		...
		if (netDriver) {
			...
			FString ipString = (char*)ArrayReaderPtr->GetData();
			// Somebody is trying to reach us, put their IP in the NAT table
			ISocketSubsystem* SocketSubSystem = netDriver->GetSocketSubsystem();
			auto connectorAddress = SocketSubSystem->CreateInternetAddr();

			FString Left, Right;
			ipString.Split(TEXT(":"), &Left, &Right);
			bool IsValid;
			connectorAddress->SetIp(*Left, IsValid);
			connectorAddress->SetPort(FCString::Atoi(*Right));
			  
			 
			FBitWriter AckPacket(227);
			...
			FPacketAudit::AddStage(TEXT("PostPacketHandler"), AckPacket);
			AckPacket.WriteBit(1);
			 

			FOutPacketTraits traits;
			netDriver->LowLevelSend(connectorAddress, AckPacket.GetData(), AckPacket.GetNumBits(), traits);

		}
	}
}

void FOnlineSessionPython::RequestPunchThrough()
{ 
	for (const FWorldContext& PieContext : GEngine->GetWorldContexts()) {

		if (PieContext.PendingNetGame) {

			const auto& clientDriver = PieContext.PendingNetGame->GetNetDriver();
			if (clientDriver) {
				ISocketSubsystem* SocketSubSystem = clientDriver->GetSocketSubsystem();
				auto STUNserverAddr = SocketSubSystem->CreateInternetAddr();
				...
				FOutPacketTraits traits;
				// We need to stop this timer when it has come through.
				clientDriver->LowLevelSend(STUNserverAddr, TCHAR_TO_UTF8 (*ipPunch), ipPunch.Len() * 8, traits);
			}
		}
	}
}
...
void FOnlineSessionPython::SetPortFromNetDriver(const FOnlineSubsystemPython& Subsystem, const TSharedPtr<FOnlineSessionInfo>& SessionInfo)
{
	...
	if (GEngine)
	{
		...
		url.Split(":", &LString, &RString, ESearchCase::IgnoreCase);
